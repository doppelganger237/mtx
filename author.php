...
<article class="alert">
		...
		if (empty($excerpt)) {
            echo deel_strimwidth(strip_tags(apply_filters('the_content', strip_shortcodes($post->post_content))) , 0, _mtx('excerpt_length') ? _mtx('excerpt_length') : 100 , '……');
        } else {
            echo deel_strimwidth(strip_tags(apply_filters('the_excerpt', strip_shortcodes($post->post_excerpt))) , 0, _mtx('excerpt_length') ? _mtx('excerpt_length') : 100 , '……');
        } ?></p>
        </article>
<?php
	}
