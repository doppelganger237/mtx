...
//获取设置选项
function _mtx($id, $returnDefault = false)
{
...
//设置页面模板
function mtx_theme_options_page()
{
	global $theme_options, $current_theme;
?>

	<div class="wrap">
		<h2>MTX 主题选项 <a href="https://support.qq.com/products/51158" class="feedback add-new-h2" target="_blank">问题反馈</a></h2>
		<div class="wp-filter">
			...
			?>
					...
					<tr id="row-<?php echo $id; ?>">
						...
						<td>
							...
							?>
									<label>
										<input name="<?php echo $id; ?>" class="regular-text" id="<?php echo $id; ?>" type="text" value="<?php echo esc_attr(_mtx($id)) ?>" />
									</label>
								...
								?>
									<label>
										...
										<input name="<?php echo $id; ?>" class="small-text" id="<?php echo $id; ?>" type="number" value="<?php echo esc_attr(_mtx($id)) ?>" />
								...
								?>
									...
									<p><textarea name="<?php echo $id; ?>" id="<?php echo $id; ?>" rows="10" cols="50" class="large-text code"><?php echo esc_textarea(_mtx($id)) ?></textarea></p>
								<?php
									break;
								case 'radio':
								?>
									<fieldset>
										<?php foreach ($option['options'] as $val => $name) : ?>
											<label>
												<input type="radio" name="<?php echo $id; ?>" id="<?php echo $id . '_' . $val; ?>" value="<?php echo $val; ?>" <?php checked(_mtx($id), $val); ?>>
												<?php echo $name; ?>
											</label>
										<?php endforeach; ?>
									</fieldset>
								...
								?>
									<label>
										<input type='checkbox' name="<?php echo $id; ?>" id="<?php echo $id; ?>" value="1" <?php echo checked(_mtx($id)); ?> />
								...
								?>
									<label>
										<input name="<?php echo $id; ?>" class="regular-text" id="<?php echo $id; ?>" type="<?php echo $type; ?>" value="<?php echo esc_attr(_mtx($id)) ?>" />
									</label>
									<p class="description"><?php echo $option['desc']; ?></p>
					<?php
									break;
					...
					<div class="panel" id="panel_about">
						<table class="form-table">
							...
							<tr>
								...
								<td>
									<ul>
										<li>QQ：1013370209（推荐）</li>
										<li>邮箱：<a href="mailto:admin@meitianxue.net">admin@meitianxue.net</a></li>
										<li>
							...
							<tr>
								...
								<td>
									<ul>
										<li>主题发布页面：<a target="_blank" href="https://meitianxue.net">https://meitianxue.net</a></li>
									</ul>
								</td>
							</tr>
							<tr>
								...
								<td>
									<ul>
										...
										<li>云落、露兜、畅萌、小影、大前端、知更鸟、yusi等等</li>
									</ul>
								</td>
							</tr>
						</table>
...
{
	if (!get_option('git_options_setup')) echo '<div class="error"><p><b>新主题已启用。该主题支持选项，请访问<a href="admin.php?page=mtx-theme-options">主题选项</a>页面进行配置。<a href="admin.php?page=mtx-theme-options">立即配置</a></b></p></div>';
}
...
function git_add_theme_options_page()
{
	...
	if (isset($_POST['action']) && isset($_GET['page']) && $_GET['page'] == 'mtx-theme-options') {
		$action = $_POST['action'];
		switch ($action) {
			case 'update':
				$_POST['uid'] = uniqid();
				...
				git_update_options();
				header('Location: admin.php?page=mtx-theme-options&update=true&panel=' . $_POST['panel']);
				break;
			case 'reset':
				delete_option('git_options_setup');
				git_update_options();
				header('Location: admin.php?page=mtx-theme-options&reset=true&panel=' . $_POST['panel']);
				break;
			case 'test':
				wp_mail(get_bloginfo('admin_email'), '[TEST]SMTP测试邮件', 'SMTP测试内容，当您收到这封邮件的时候，证明您的网站SMTP配置已成功！');
				header('Location: admin.php?page=mtx-theme-options&test=true&panel=' . $_POST['panel']);
				break;
		}
		exit;
	}
	add_menu_page('MTX 主题选项', '主题选项', 'manage_options', 'mtx-theme-options', 'mtx_theme_options_page', 'dashicons-universal-access-alt');
}
