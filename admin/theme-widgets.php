...
class git_banner extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct('git_banner', 'MTX-广告', $widget_ops);
    }
...
class git_comment extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_comment', 'MTX-最新评论', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
...
class git_postlist extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_postlist', 'MTX-聚合文章', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
...
function githeme_posts_list($orderby, $limit, $cat, $img) {
    ...
    while (have_posts()):
        ...
        ?>
<li>
  <a target="_blank" href="<?php  the_permalink(); ?>" title="<?php the_title(); ?>"><span class="thumbnail"><?php  echo post_thumbnail(_mtx('cdn_url_style') ? '!githumb1.jpg' : '?imageView2/1/w/100/h/64/q/75',true);?></span>
    <span class="text"><?php the_title(); ?>
    </span>
    <span class="muted"><?php   the_time('Y-m-d'); ?>
    </span>
...
class git_reader extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_reader', 'MTX-活跃读者', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
...
class git_rec extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_rec', 'MTX-推荐模块', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
...
class git_slick extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_slick', 'MTX-幻灯片(风格二)', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
...
class git_slide extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_slide', 'MTX-幻灯片(风格一)', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $img1 = $instance['img1'];
...
class git_social extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_social', 'MTX-社交按钮', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
        ...
        echo '<div class="social-about"><div class="social-img"><img data-original="'.$img.'"></div>';
        echo '<div class="social-title"><h3>'.$name.'</div></h3>';
        echo '<div class="social-text">'.$description.'</div></div>';
       
       echo '<ul class="social-icon">';
         if (_mtx('mtx_weixin_qr'))
            echo '<li><a class="weixin"><i class="weixins fa fa-weixin"></i><div class="weixin-popover"><div class="popover bottom in"><div class="arrow"></div><div class="popover-title">加微信</div><div class="popover-content"><img width="200px" height="200px" src="' . _mtx('mtx_weixin_qr') . '" ></div></div></div></a></li>';
        if (_mtx('mtx_alipay_qr'))
            echo '<li><a class="weixin"><i class="pay fa fa-alipay"></i><div class="weixin-popover"><div class="popover bottom in"><div class="arrow"></div><div class="popover-title">支付宝</div><div class="popover-content"><img src="' . _mtx('mtx_alipay_qr') . '" ></div></div></div></a></li>';
            if (_mtx('mtx_customicon_name'))
            echo '<li><a  href="' . _mtx('mtx_customicon_url') . '" rel="external nofollow" title="' . _mtx('mtx_customicon_name') . '" target="_blank"><i class="customicon fa ' . _mtx('mtx_customicon_icon') . '"></i></a></li>';
     
            if (_mtx('mtx_qq'))
            echo '<li><a class="weixin" href="tencent://message/?uin=' . _mtx('mtx_qq') . '&Site=&Menu=yes " rel="external nofollow" title="联系QQ" target="_blank"><i class="qq fa fa-qq"></i><div class="weixin-popover"><div class="popover bottom in"><div class="arrow"></div><div class="popover-title">加QQ</div><div class="popover-content"><img width="200px" height="200px" src="' . _mtx('mtx_qq_qr') . '" ></div></div></div></a></li>';
        echo '</ul></div>';
        echo $after_widget;
    }
    function form($instance) {
...
<label>
    ...
    <input class="widefat" id="<?php
                echo $this->get_field_id('img'); ?>" name="<?php
                echo $this->get_field_name('img'); ?>" type="text" value="<?php
                echo $instance['img']; ?>" />
  </label>
</p>
<p>
<p>
  <label>
    ...
    <textarea id="<?php
                ...
                echo $this->get_field_name('description'); ?>" class="widefat" rows="5"><?php
                    echo $instance['description']; ?></textarea>
  </label>
</p>
<?php
    }
...
class git_subscribe extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_subscribe', 'MTX-邮箱订阅', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
...
class git_tag extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_tag', 'MTX-标签云', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
...
class git_textbanner extends WP_Widget {
    function __construct() {
        ...
        parent::__construct('git_textbanner', 'MTX-特别推荐', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args);
        $title = apply_filters('widget_name', $instance['title']);
...
class git_tongji extends WP_Widget {
    function __construct() {
        ...
        parent::__construct(false, 'MTX-网站统计', $widget_ops);
    }
    function form($instance) {
