...
public static function pay($atts, $content = null) {
    ...
    if (is_user_logged_in()) {
        if ($result == $description || current_user_can('administrator')) {
            $notice.= '<div class="alert alert-info""><p style="margin: 6px 0 5px 0;padding: 0;">';
            $notice.= $content;
            $notice.= '</p></div>';
        } else {
            if (Points::get_user_total_points($user_id, POINTS_STATUS_ACCEPTED) < $point) {
                ...
                $notice.= '<p style="color:red;">您当前拥有 <strong>' . Points::get_user_total_points($user_id, POINTS_STATUS_ACCEPTED) . '</strong> 金币，您的金币不足，可以通过评论赚钱金币</p>';
                ...
                $notice.= '<p style="color:red;">本段内容需要付费查看，您当前拥有 <strong>' . Points::get_user_total_points($user_id, POINTS_STATUS_ACCEPTED) . '</strong> 金币</p>';
...
function pay_point() {
    ajax.post("' . admin_url('admin-ajax.php') . '", "action=pay_buy&point=' . $point . '&userid=' . $user_id . '&id=' . $description . '", function(n) {
        ...
        $notice.= '<div class="alert alert-info"" ><p style="margin: 6px 0 5px 0;padding: 0;"></p>';
