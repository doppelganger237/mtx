...
switch ($ui['action']) {
    case 'signin':
        ...
        if ( is_wp_error($user_verify) ){
            print_r(json_encode(array('error'=>1, 'msg'=>$user_verify ->get_error_message())));  
            exit();  
        }
   ...
   /*      if( !preg_match('/^[a-zA-Z\d_]{3,20}$/i', $ui['name']) ) {  
        ...
        if(username_exists($ui['name'])){
            
            print_r(json_encode(array('error'=>1, 'msg'=>'用户名已存在，换一个试试')));  
            exit();
        }
        if(email_exists($ui['email'])){
        ...
        if ( is_wp_error($status) ){
            $err = $status->errors;
            // print_r($err);
            print_r(json_encode(array('error'=>1, 'msg'=>$status->get_error_message())));  
            exit();
        }
