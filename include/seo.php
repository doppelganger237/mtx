...
if (_mtx('auto_go')) {
    ...
    add_filter('the_content', 'auto_go', 999);
}
...
function deel_description() {
    ...
    if (is_singular()) {
        ...
        $description = _mtx('git_description'); // 首頁要自己加
    } elseif (is_tag()) {
...
//页面描述 d_description
if (_mtx('git_description')){
    add_action('wp_head', 'deel_description');
}

//WordPress文字标签关键词自动内链
$match_num_min = _mtx('autolink_condition'); //一篇文章中同一個標籤少於幾次不自動鏈接
$match_num_max = _mtx('autolink_max'); //一篇文章中同一個標籤最多自動鏈接幾次
