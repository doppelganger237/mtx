...
//注册之后跳转
if (_mtx('register_redirect_ok')) {
    function mtx_registration_redirect() {
        if (_mtx('redirect_choise') == '1') {
            $redirect_url = home_url();
        } elseif (_mtx('git_redirect_choise') == '2') {
            $redirect_url = get_author_posts_url($user_id);
        } elseif (_mtx('git_redirect_choise') == '3') {
            $redirect_url = admin_url('wp-admin/profile.php');
        } elseif (_mtx('git_redirect_choise') == '4' && _mtx('register_redirect_url')) {
            $redirect_url = _mtx('register_redirect_url');
        }
        return $redirect_url;
    }
    add_filter('registration_redirect', 'mtx_registration_redirect');
}
...
if (_mtx('regist_ips')) {
    function validate_reg_ips(){
        global $err_msg;
        $allow_time = _mtx('regist_ips_num');
        //每个IP允许注册的用户数
        $allowed = true;
