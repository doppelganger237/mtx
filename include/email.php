...
if (_mtx('smtp_enable')) {
    ...
    {
        $phpmailer->From = _mtx('mail_address'); //发件人地址
        $phpmailer->FromName = _mtx('mail_name'); //发件人昵称
        $phpmailer->Host = _mtx('mail_server'); //SMTP服务器地址
        $phpmailer->Port = _mtx('mail_port'); //SMTP邮件发送端口
        if (_mtx('mail_ssl')) {
            $phpmailer->SMTPSecure = 'ssl';
        } else {
            $phpmailer->SMTPSecure = '';
        } //SMTP加密方式(SSL/TLS)没有为空即可
        $phpmailer->Username = _mtx('mail_user'); //邮箱帐号
        $phpmailer->Password = _mtx('mail_password'); //邮箱密码
        $phpmailer->IsSMTP();
...
if (_mtx('user_notification_to_admin')) {
    add_filter('wp_new_user_notification_email_admin', '__return_false');
} //关闭新用户注册站长邮件
//欢迎新用户邮件
if (_mtx('user_notification_to_user')) {
